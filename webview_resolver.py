...
(function() {
    ...
    function clickLikelyButton() {
        var words = [
            "devam", "devam et", "continue", "go", "get link", "getlink",
            "link", "skip", "gec", "geç", "git", "open", "verify"
        ];

        var els = Array.prototype.slice.call(document.querySelectorAll(
            "button, a, input[type=button], input[type=submit], .btn, .button, [role=button], [onclick]"
        ));
        ...
        for (var i=0; i<els.length; i++) {
            ...
            for (var j=0; j<words.length; j++) {
                if (text.indexOf(words[j]) >= 0) score += 10;
            }
            if ((el.href || "").indexOf("javascript") >= 0) score += 3;
            if (el.onclick) score += 3;
            if (text.indexOf("btn") >= 0) score += 1;

            if (score > 0) candidates.push({el: el, score: score, text: text});
        }
    ...
    setTimeout(scan, 6200);
    setTimeout(clickLikelyButton, 8000);
    setTimeout(scan, 9000);
    setTimeout(clickLikelyButton, 11000);
...
class HiddenWebViewResolver:
    def __init__(self, app, start_url: str, timeout_seconds: int = 120):
        self.app = app
    ...
    def start_visible(self):
        """Open visible WebView fallback.

        User can complete Cloudflare/robot/green button manually.
        Ads/popunders are ignored, and final file/cloud URL is captured.
        """
        self.visible = True
        try:
        ...
        class CloseClick(PythonJavaClass):
        ...
        def _open():
            try:
                ...
                try:
                    resolver.original_view = content.getChildAt(0)
                except Exception:
                    resolver.original_view = None

                container = LinearLayout(activity)
                container.setOrientation(LinearLayout.VERTICAL)
                container.setBackgroundColor(Color.rgb(5, 5, 10))

                close_btn = Button(activity)
                close_btn.setText("Web modu kapat")
                close_btn.setOnClickListener(CloseClick())
                ...
                container.addView(close_btn, LayoutParams(LayoutParams.MATCH_PARENT, 120))
                container.addView(webview, LayoutParams(LayoutParams.MATCH_PARENT, 0, 1.0))

                # Kivy ekraninin ustune view eklemek bazi cihazlarda gorunmuyor.
                # Tam ekran WebView icin direkt Activity content view degistiriyoruz.
                ...
                try:
                    resolver.app.on_hidden_webview_debug("Gorunur WebView gercek ekrana acildi")
                except Exception:
                    pass
            except Exception as exc:
                resolver.app.on_hidden_webview_error("Gorunur WebView hatasi: " + str(exc))

        _open()

        try:
            from kivy.clock import Clock
            self.event = Clock.schedule_interval(lambda _dt: self.poll(), 0.75)
            self.timeout_event = Clock.schedule_once(lambda _dt: self.timeout(), self.timeout_seconds)
            self.app.on_hidden_webview_debug("Gorunur poll basladi")
        except Exception as exc:
            self.app.on_hidden_webview_error("Poll baslatma hatasi: " + str(exc))

    def start(self):
        # Direct visible resolver. Keep this method for compatibility with older main.py.
        return self.start_visible()
    ...
    def poll(self):
        ...
        self.ticks += 1
        try:
            self.app.on_hidden_webview_progress(self.start_url, self.ticks)
        except Exception:
            pass
        ...
        if current:
            try:
                self.app.on_hidden_webview_debug("URL: " + current[:90])
            except Exception:
                pass
        ...
        try:
            if self.ticks in (1, 3, 6, 9, 12):
                self.app.on_hidden_webview_debug("JS enjekte ediliyor")
        except Exception:
            pass
    ...
    def cleanup(self):
        ...
        def _clean():
            ...
            try:
                PythonActivity = __import__("jnius").autoclass("org.kivy.android.PythonActivity")
                activity = PythonActivity.mActivity
                if resolver.original_view is not None:
                    activity.setContentView(resolver.original_view)
            except Exception:
                try:
