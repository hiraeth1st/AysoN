    ...
    def start(self):
        ...
        def _open():
            try:
                ...
                # Full-size WebView is needed because some gates do not activate
                # timers/buttons correctly in a tiny 2x2 viewport.
                # Keep it nearly invisible so user does not need to interact.
                ...
                try:
                    resolver.app.on_hidden_webview_debug("WebView olustu, sayfa yukleniyor")
                except Exception:
                    pass
            except Exception as exc:
        ...
        # Kivy Clock'u Android UI thread disindan baslat.
        # Bazi cihazlarda UI thread icinden schedule_interval calisinca poll donmuyor.
        try:
            from kivy.clock import Clock
            self.event = Clock.schedule_interval(lambda _dt: self.poll(), 1.0)
            self.timeout_event = Clock.schedule_once(lambda _dt: self.timeout(), self.timeout_seconds)
            self.app.on_hidden_webview_debug("Poll basladi")
        except Exception as exc:
