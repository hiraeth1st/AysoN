...
class AysonApp(App):
    ...
    def _show_success(self, result):
        ...
        if isinstance(final, str) and final.startswith(WEBVIEW_REQUIRED_PREFIX):
            ...
            self.output.text = (
                ...
                "Gizli WebView resolver otomatik calisiyor...\n"
                "JS sayaci bekleniyor, buton otomatik tiklanacak, reklam sekmesi yok sayilacak.\n\n"
                + web_url
            )
            self.set_status("Otomatik WebView", WARNING)
    ...
    def start_hidden_webview_resolver(self, url):
        ...
        try:
            self.hidden_webview_resolver = HiddenWebViewResolver(self, url, timeout_seconds=35)
            self.hidden_webview_resolver.start()
        except Exception as exc:
            self.output.text += "\n\nWebView resolver hatasi:\n" + str(exc)
    ...
    def on_hidden_webview_failed(self, source_url):
        ...
        self.output.text = (
            "Ara link bulundu:\n"
            + source_url
            + "\n\nOtomatik WebView resolver final linki yakalayamadi.\n"
            "Bu lnk.news kapisi daha sert JS/Cloudflare kontrolu kullaniyor olabilir."
        )
        self.set_status("Manuel devam gerekli", WARNING)
        self.set_open_button(source_url)
